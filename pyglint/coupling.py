...
def atm_to_ism(smb_atm, sfct_atm, topo_atm, area_atm,  grid_atm,
    ...
    mask = np.where(frac_ism > 0.01, 1, 0)
    
    sfct_ism, smb_ism_nc = \
      (mask*interp_to_surface(f_xyz, topo_xyz, topo_ism) \
       for f_xyz in (sfct_xyz, smb_xyz))
          
    # \todo : conservation enforcement - see glint_downscaling_gcm
    # roughly - adjust smb so that the total smb_ism within an 
    # atmosphere grid box equals the smb_atm integrated vertically.
    
    # this resembles the glint code. blocky. there is a NASTY! comment in the glint code
    smb_ism = np.zeros(smb_ism_nc.shape) + smb_ism_nc
    if True and not type(area_atm) is None:
    
        ism_to_atm_map = local_to_global_map(down_transform, grid_atm, grid_ism)
        plt.pcolormesh(ism_to_atm_map%7,cmap='tab20c')
        plt.show()
        ...
        for I in grid_atm.axes_index[0]:
            for J in grid_atm.axes_index[1]:
               if np.any(valid_atm[:,J,I]):
                ...
                #indx = mask & ( ism_to_atm_map == cell_id(I,J,*grid_atm.shape))
    ...
    return smb_ism, smb_ism_nc, sfct_ism 
...
#%% downscaling & upscaling   
    smb_ism, smb_ism_nc, sfct_ism = atm_to_ism(smb_atm, sfct_atm, topo_atm, area_atm,
